...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['AGU_TOKEN_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['AGU_TOKEN_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Agu Token API.
 */
export class AguToken {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['AGU_TOKEN_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('AGU_TOKEN_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AguTokenError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the AguToken client with an apiKey option, like new AguToken({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? AguToken.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('AGU_TOKEN_LOG'), "process.env['AGU_TOKEN_LOG']", this) ??
      defaultLogLevel;
  ...
  static AguToken = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static AguTokenError = Errors.AguTokenError;
...
AguToken.PetResource = PetResource;
AguToken.Store = Store;
AguToken.UserResource = UserResource;
export declare namespace AguToken {
